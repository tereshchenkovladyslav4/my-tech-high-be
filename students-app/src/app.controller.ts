...
export class AppController {
  ...
  async downloadStudentRecordFiles(
    @Param('downloadItems') param: string,
  ): Promise<StreamableFile> {
    try {
      const downloadItems: DownloadStudentRecordFilesVM[] = JSON.parse(param);
      if (downloadItems.length > 0) {
        var req = require('request-promise').defaults({ encoding: null });
        if (
          downloadItems.length == 1 &&
          downloadItems[0].fileIds?.length == 1 &&
          downloadItems[0].isIndividualFile
        ) {
          const file = await this.fileService.findOneById(
            downloadItems[0].fileIds[0],
          );
          let data = req.get(file?.signedUrl);
          return new StreamableFile(data);
        } else {
          const AdmZip = require('adm-zip');
          const zip = new AdmZip();
          let dataToArchive: { url: String; fileName: string }[] = [];

          for (let i = 0; i < downloadItems.length; i++) {
            const files = await this.fileService.findByIds(
              downloadItems[i].fileIds.join(','),
            );
          ...
          let PromiseArray = dataToArchive.map((dataItem) => {
        ...
        throw new HttpException(
          "You don't have permission to download files at storage!",
