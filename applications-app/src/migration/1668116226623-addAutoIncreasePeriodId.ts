...
export class addAutoIncreasePeriodId1668116226623 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE mth_provider_period ADD FOREIGN KEY (\`period_id\`) REFERENCES mth_period(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE mth_subject_period ADD FOREIGN KEY (\`period_id\`) REFERENCES mth_period(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE mth_schedule_period ADD FOREIGN KEY (\`PeriodId\`) REFERENCES mth_period(\`id\`)`);
    }
